  ...
  function addKeyEffect (req, res, next) {
    ...
    switch (effect) {
      ...
      case 'gradient':
        ...
        break
      default:
        validateEffect = Promise.reject(new CkbError('invalidEffectType', undefined, effect))
    }
...
function validateAddGradientRequest (body) {
  ...
  if (duration !== undefined && (typeof duration !== 'number' || duration <= 0)) {
    return Promise.reject(new CkbError('invalidDuration', null, duration))
  } else if (loopCount !== undefined && (typeof loopCount !== 'number' || !Number.isInteger(loopCount) || loopCount < 0)) {
    return Promise.reject(new CkbError('invalidLoopCount', null, loopCount))
  } else if (!Array.isArray(colorStops)) {
    return Promise.reject(new CkbError('invalidColorStops', null, colorStops))
  }
  ...
  if (errors.length > 0) {
    return Promise.reject(new CkbError('invalidColorStops', null, errors))
  }
...
function validateAddColorRequest (body) {
  ...
  if (!color) return Promise.reject(new CkbError('invalidColor', null, body.color))
